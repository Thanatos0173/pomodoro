...
def play_sound_1():
    pygame.mixer.music.load("killer_queen_button_trigger.mp3")
...
def play_sound_3():
    pygame.mixer.music.load("cell_phone_vibration.mp3")
    pygame.mixer.music.play()
...
def update(window,canvas,time):
    canvas.delete("all")
    seven_segment_display_digit(canvas,30,50,digitToTable(time[-6]))
    seven_segment_display_digit(canvas,130,50,digitToTable(time[-5]))
    seven_segment_display_digit(canvas,250,50,digitToTable(time[-4]))
    seven_segment_display_digit(canvas,350,50,digitToTable(time[-3]))
    seven_segment_display_digit_small(canvas,450,110,digitToTable(time[-2]))
    seven_segment_display_digit_small(canvas,500,110,digitToTable(time[-1]))
...
def update2(window,canvas,doWork):
    canvas.delete("all")
    if doWork:
        wordToSegmentDisplay(canvas,"work",200,25,"green")
    else:
        wordToSegmentDisplay(canvas,"rest",200,25,"green")
...
def update3(window,canvas):
    canvas.delete("all")
    global isPaused
    wordToSegmentDisplay(canvas,"pause",10,25, "green")
    wordToSegmentDisplay(canvas,"resume",280,25,"green")

    window.update()


def pause(window,canvas):
    global isPaused
    isPaused = True

def resume(window,canvas):
    global isPaused
    isPaused = False


def clockwatch(window,duration,canvas):
    ...
    while True:
        ...
        if current_second != last_second: # A second elapsed
            ...
            if not isPaused:
                currentTime -= 0.01
                listToAdd = timeToSegmentConvertable(int(currentTime))
...
def on_enter(event,canvas):
    ...
    if is_left:
        wordToSegmentDisplay(canvas,"pause",10,25, "red")
    else:
        wordToSegmentDisplay(canvas,"resume",280,25, "red")
...
def on_click(event,window, canvas):
    half_clicked = "Left" if event.x <= canvas.winfo_width() // 2 else "Right"
    if half_clicked == "Left" and not isPaused:
        pause(window,canvas)
        play_sound_1()
    elif half_clicked == "Left" and isPaused:
        pause(window,canvas)
        play_sound_3()
    elif half_clicked == "Right" and isPaused:
        resume(window,canvas)
        play_sound_1()
    elif half_clicked == "Right" and not isPaused:
        resume(window,canvas)
        play_sound_3()
    else: None
...
def start():
    ...
    canvas3.bind('<Enter>', lambda event: on_enter(event,canvas3))
    canvas3.bind('<Leave>', lambda event: on_leave(event,canvas3))
    canvas3.bind('<Motion>', lambda event: on_motion(event,canvas3))
    canvas3.bind('<Button-1>', lambda event: on_click(event,window, canvas3))

    while True:
        for i in range(3):
            update2(window,canvas2,True)
            clockwatch(window,25*60,canvas)
            update2(window,canvas2,False)
            clockwatch(window,300,canvas)
        update2(window,canvas2,True)
        clockwatch(window,25*60,canvas)
        update2(window,canvas2,False)
        clockwatch(window,15*60,canvas)
